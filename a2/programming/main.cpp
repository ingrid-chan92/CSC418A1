...
void initGlui()
{
	...
	glui_spinner->set_float_val(90.0f);
	glui_spinner->set_speed(SPINNER_SPEED);
...
void drawBeak()
{
	...
	glTranslatef(0.0f, -10.0f * joint_ui_data->getDOF(Keyframe::BEAK), 0.0f);
...
void writePGM(char* filename, GLubyte* buffer, int width, int height, bool raw=true)
{
	...
	else
	{
		...
		for(int y=height-1;y>=0;y--)
		{
			for(int x=0;x<width;x++)
			{
				fprintf(fp,"%ld ",intptr_t(&buffer[x+y*width]));
			}
			fprintf(fp,"\n");
		}
	}
